...
{ 
	...
	public void payCash()
	{
		...
		for (int i = 0; i < cart.nameLists.size(); i++)
			totPrice += ( cart.amountLists.get(i) * cart.amountLists.get(i) );
...
//==================================================================================================//
	...
	public void payPoint()
	{
		...
		for (int i = 0; i < cart.nameLists.size(); i++)
			cart.totPrice += ( cart.amountLists.get(i) * cart.priceLists.get(i) );  // 수량 * 상품가격
		...
		if (point < totPrice)	// → 충전 후 진행
		{
			...
			totPrice = 0; // 고른 총액 다 초기화.
			cart.clearCart2(); // 장바구니 비운 후 유저메뉴 출력
	...
	private void receipt()
	{
		...
		int a = 0;
		for (int i = 0; i < cart.nameLists.size(); i++)
			System.out.printf("%-10s\t%10d\t%10d\n", cart.nameLists.get(i), cart.amountLists.get(i), cart.priceLists.get(i));
	...
	public void calCateTot()
	{
		...
		for (int i = 0; i < cart.nameLists.size(); i++)
		{	
			category = pdList.get(cart.nameLists.get(i)).getCategory();

			if (category == 1)
				salesCate1 += cart.amountLists.get(i) * cart.priceLists.get(i);
			else if (category == 2)
				salesCate2 += cart.amountLists.get(i) * cart.priceLists.get(i);
			else	// category == 3
				salesCate3 += cart.amountLists.get(i) * cart.priceLists.get(i);
		}	
